...
CODEIUM_FILE="$REPO_ROOT/CODEIUM.md"
TABNINE_FILE="$REPO_ROOT/TABNINE.md"
CODEWHISPERER_FILE="$REPO_ROOT/CODEWHISPERER.md"
GHOSTWRITER_FILE="$REPO_ROOT/GHOSTWRITER.md"
CODIUM_FILE="$REPO_ROOT/CODIUM.md"
...
update_agent_file() {
    ...
    if [ ! -f "$target_file" ]; then
        ...
        elif [ -f "$REPO_ROOT/templates/IFLOW.md" ] && [ "$agent_name" = "iFlow CLI" ]; then
            cp "$REPO_ROOT/templates/IFLOW.md" "$temp_file"
        else
...
case "$AGENT_TYPE" in
    ...
    "cursor")
        ...
        ;;
    "codeium")
        update_agent_file "$CODEIUM_FILE" "Codeium"
        ;;
    "tabnine")
        update_agent_file "$TABNINE_FILE" "Tabnine"
        ;;
    "codewhisperer")
        update_agent_file "$CODEWHISPERER_FILE" "Amazon CodeWhisperer"
        ;;
    "ghostwriter")
        update_agent_file "$GHOSTWRITER_FILE" "Replit Ghostwriter"
        ;;
    "codium")
        update_agent_file "$CODIUM_FILE" "CodiumAI"
        ;;
    "iflow")
    ...
    "")
        ...
        [ -f "$CODEIUM_FILE" ] && update_agent_file "$CODEIUM_FILE" "Codeium"
        [ -f "$TABNINE_FILE" ] && update_agent_file "$TABNINE_FILE" "Tabnine"
        [ -f "$CODEWHISPERER_FILE" ] && update_agent_file "$CODEWHISPERER_FILE" "Amazon CodeWhisperer"
        [ -f "$GHOSTWRITER_FILE" ] && update_agent_file "$GHOSTWRITER_FILE" "Replit Ghostwriter"
        [ -f "$CODIUM_FILE" ] && update_agent_file "$CODIUM_FILE" "CodiumAI"
        [ -f "$IFLOW_FILE" ] && update_agent_file "$IFLOW_FILE" "iFlow CLI"
        
        # If no files exist, create based on current directory or ask user
        if [ ! -f "$CLAUDE_FILE" ] && [ ! -f "$GEMINI_FILE" ] && [ ! -f "$COPILOT_FILE" ] && [ ! -f "$CURSOR_FILE" ] && [ ! -f "$CODEIUM_FILE" ] && [ ! -f "$TABNINE_FILE" ] && [ ! -f "$CODEWHISPERER_FILE" ] && [ ! -f "$GHOSTWRITER_FILE" ] && [ ! -f "$CODIUM_FILE" ] && [ ! -f "$IFLOW_FILE" ]; then
            echo "No agent context files found. Creating Claude Code context file by default."
            update_agent_file "$CLAUDE_FILE" "Claude Code"
        fi
        ;;
    *)
        echo "ERROR: Unknown agent type '$AGENT_TYPE'. Use: claude, gemini, copilot, cursor, codeium, tabnine, codewhisperer, ghostwriter, codium, iflow, or leave empty for all."
        exit 1
        ;;
esac
echo ""
...
echo ""
echo "Usage: $0 [claude|gemini|copilot|cursor|codeium|tabnine|codewhisperer|ghostwriter|codium|iflow]"
...
echo "  - codeium: Update only CODEIUM.md"
echo "  - tabnine: Update only TABNINE.md"
echo "  - codewhisperer: Update only CODEWHISPERER.md"
echo "  - ghostwriter: Update only GHOSTWRITER.md"
echo "  - codium: Update only CODIUM.md"
echo "  - iflow: Update only IFLOW.md"